...
`recent-research`는 최근 30일 전후의 공개 시그널을 모아 짧은 근거 기반 브리프를 만드는 제품형 스킬입니다. GitHub, Hacker News, Reddit, Google News RSS를 API 키 없이 조회하고, 결과를 `brief`, `markdown`, `json`, `html` 형식으로 출력합니다.

주로 이런 상황에서 씁니다.

- 제품, 경쟁사, 오픈소스, 개발자 도구의 최근 반응을 빠르게 보고 싶을 때
- `Codex vs Cursor`처럼 여러 대상을 같은 기준으로 비교하고 싶을 때
- 회의 전 최근 GitHub 활동, 커뮤니티 반응, 뉴스 헤드라인을 근거와 함께 정리하고 싶을 때
- 조사 결과를 HTML 파일로 저장해 공유하고 싶을 때
...
python3 scripts/research.py "Codex vs Cursor" --days 30 --emit html --save
...
`workflow`는 작업 상태를 repo-local `.agent/`에 남깁니다. `.agent/`는 기본적으로 git에 포함해 다음 에이전트와 체크아웃에서도 같은 상태를 이어받습니다.
